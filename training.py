...
class Trainer(abc.ABC):
    ...
    def fit(self, dl_train: DataLoader, dl_test: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            losses, accuracy = train_result
            train_loss.extend(losses)
            train_acc.append(accuracy)
            test_result = self.test_epoch(dl_test, **kw)
            losses, accuracy = test_result
...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        y_pred_prob, self.h_s = self.model(x, self.h_s)
        loss = self.loss_fn(torch.transpose(y_pred_prob, 1, 2), y)
        self.optimizer.zero_grad()
        loss.backward(retain_graph=True)
        self.optimizer.step()
        self.h_s = self.h_s.detach()
        y_pred = y_pred_prob.argmax(dim=2)
...
class MyTrainer(Trainer):
    def train_batch(self, batch) -> BatchResult:
        ...
        loss, data_loss, _ = self.loss_fn(x, x_r, mu, log_sigma2)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
